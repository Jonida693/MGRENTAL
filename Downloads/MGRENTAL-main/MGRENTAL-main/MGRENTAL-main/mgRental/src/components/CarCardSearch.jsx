  ...
  const [autoImgError, setAutoImgError] = useState(false);
    ...
    return (
      <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden
        ...
        {/* Left image column (fills column height) */}
        <div
          className="w-full md:w-56 flex-shrink-0 h-56 md:h-56 bg-center bg-cover bg-no-repeat rounded-t-xl md:rounded-l-xl"
          style={{ backgroundImage: `url(${image})`, backgroundColor: '#f8fafc' }}
        />
          ...
          <div className="mt-2 grid grid-cols-2 md:grid-cols-2 gap-4 text-sm text-gray-600">
            ...
            <div className="space-y-2 text-right">
              <div className="flex items-center justify-end gap-3">
                {!autoImgError ? (
                  <img src={autoIcon} alt="transmission" className="h-5 w-5 object-contain" onError={() => setAutoImgError(true)} />
                ) : (
